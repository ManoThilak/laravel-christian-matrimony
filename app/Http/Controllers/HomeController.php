...
class HomeController extends Controller
{
    ...
    public function member_listing(Request $request)
    {
        ...
        $complexion     = ($request->complexion != null) ? $request->complexion : null;
        ...
        if(!empty($complexion)){
            $user_ids = PhysicalAttribute::where('complexion',  $complexion)->pluck('user_id')->toArray();
          //  if(count($user_ids) >0){
                $users = $users->WhereIn('id', $user_ids);
          //  }
        }
        ...
        return view('frontend.member.member_listing.index', compact('users','age_from','age_to','member_code','matital_status','religion_id','caste_id','sub_caste_id','mother_tongue','profession','country_id','state_id','city_id','min_height','max_height','complexion','member_type'));
    }
    ...
    public function update_email(Request $request)
    {
        ...
        $user->email = $email;
        $user->save();
        flash(translate('Email Succesfully Updated .'))->success();
            return back();
        }
