    ...
    <form action="{{ route('member.basic_info_update', $member->id) }}" method="POST">
        ...
        <div class="form-group row">
            ...
            <div class="@if(addon_activation('otp_system')) col-md-6 @else col-md-12 @endif">
                ...
                <input type="number" name="email" value="{{ $member->email }}" class="form-control" placeholder="{{translate('phone')}}" disabled>
                @error('email')
                    <small class="form-text text-danger">{{ $message }}</small>
                @enderror
            </div>
            {{-- @if(addon_activation('otp_system')) --}}
              <div class="col-md-6" style="display:none;">
                  ...
                  <input type="number" minlength="10" maxlength="10" name="phone" required value="{{ $member->phone }}" class="form-control" placeholder="{{translate('Phone')}}">
                  @error('phone')
                      <small class="form-text text-danger">{{ $message }}</small>
                  @enderror
              </div>
