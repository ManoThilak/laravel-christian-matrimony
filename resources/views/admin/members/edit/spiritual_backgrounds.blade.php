...
<div class="card-body">
    <form action="{{ route('spiritual_backgrounds.update', $member->id) }}" method="POST">
        ...
        <div class="form-group row">
            ...
            $reid = !empty($member->spiritual_backgrounds->family_value_id) ? $member->spiritual_backgrounds->family_value_id : "";
            
            @endphp
             <div class="col-md-6">
                  <label for="family_value_id">{{translate('Family Value')}}</label>
                  <select class="form-control aiz-selectpicker" name="family_value_id" id="family_value_id" data-live-search="true">
                      <option value="">{{translate('Select One')}}</option>
                      @foreach ($family_values as $family_value)
                        

                         <option value="{{$family_value->id}}" @if($family_value->id == $reid) selected @endif >{{$family_value->name}}</option>
